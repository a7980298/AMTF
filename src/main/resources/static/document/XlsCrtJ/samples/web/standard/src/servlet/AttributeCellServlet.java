...
public class AttributeCellServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private String result;
	private String outFileName;
	private XlsxCreator xlsxCreator = new XlsxCreator();

	public AttributeCellServlet() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse  response) throws ServletException, IOException {
		...
		// セルの書式設定
		if (request.getParameter("attributecell") != null)
		{
			// 日付をキーとしたファイル名を生成
			SimpleDateFormat simpleDateFormat = new SimpleDateFormat("'attributecell'_yyyyMMdd_HHmmss'.xlsx'");
			...
			if (request.getParameter("format") != null) {
				...
				xlsxCreator.getCell("B5").setValue(time);
				// パーセンテージ
			...
			// 罫線
			if (request.getParameter("ruledline") != null) {
				// 「罫線」用のシートを作成
				AddSheet("罫線");
				xlsxCreator.setZoom(85);
				xlsxCreator.getCell("A1").getAttr().setFontPoint(14);
