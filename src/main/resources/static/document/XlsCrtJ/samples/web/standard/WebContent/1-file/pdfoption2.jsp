...
<body>
	...
	<form id="form1" action="<%=request.getContextPath()%>/PdfOption2Servlet" method="post">
		<!-- Contents Main -->
		<div class="column-contents">
			...
			<div class="column-contents-1">
				<article class="article">
					...
					<h2>
						<i class="fa fa-chevron-right"></i>ファイル作成（PDF オプション 2）
					</h2>
					<p>出力 PDF ファイルにフォントの埋め込みを行います。</p>
					<p><i class="fa fa-square"></i>フォント・外字ファイルの埋め込み<br /></p>
					<p>PDF ファイルにフォントを埋め込む事で、特定のフォントがない環境での PDF ファイル閲覧に対応します。</p>
					<ul>
						<li class="check-square"><i class="fa fa-check"></i>
							<label style="display:inline-block;width:180px;">フォントの埋め込み</label>
							<select name="embedfont" size="1">
								<option value="0">埋め込まない</option>
								<option value="1">特定のフォントを指定</option>
								<option value="2">すべてのフォント</option>
							</select>
						</li>
					</ul>
					<p>外字の扱いについて、既定では OS に登録された外字が埋め込まれます。任意の外字ファイルが指定できる事で、環境に依存しない外字登録が実現します。</p>
					<ul>
						<li class="check-square"><i class="fa fa-check"></i>
							<label style="display:inline-block;width:180px;">外字ファイルの埋め込み</label>
							<select name="embedeudcfiles" size="1">
								<option value="0">埋め込まない</option>
								<option value="1">フォント名を指定</option>
								<option value="2">すべてのフォント</option>
							</select>
						</li>
					</ul>
					...
					<p>
						<input type="submit" name="pdfembed" value="PDF ファイル作成" style="width: 150px;" />
					</p>

					<p class="annotation1">※サンプルコードは「src/servlet/PdfOption2Servlet.java」を参照してください。</p>
