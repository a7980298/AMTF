...
<body>
	...
	<form id="form1" action="<%=request.getContextPath()%>/CalculateServlet" method="post">
		<!-- Contents Main -->
		<div class="column-contents">
			...
			<div class="column-contents-1">
				<article class="article">
					...
					<h2>
						<i class="fa fa-chevron-right"></i>Excel 計算式
					</h2>
					<p>ExcelCreator の機能で Excel 計算式関連の設定を行います。</p>
					<p>
						<i class="fa fa-square"></i>計算式の設定
					</p>
					<p>setFunc メソッドで新規に計算式を設定します。</p>
					<p>
						サンプルの処理では任意の計算結果(100)を指定しているため、生成 Excel ファイルオープン時は指定した値がそのまま結果として表示されます。<br />Excel 側の機能で再計算された計算結果を表示させる場合、setFullCalcOnLoad メソッドを使用して再計算実行フラグを設定します。
					</p>
					<p>
						<input type="checkbox" name="fullcalconload" id="c1" value="fullcalconload" width="200px" /><label for="c1">setFullCalcOnLoad</label>
					</p>
					<p>
						<input type="submit" name="calculate1" value="Excel ファイル作成" style="width: 150px;" />
					</p>
					<hr />
					<p>
						<i class="fa fa-square"></i>計算式セルのコピー・移動
					</p>
					<p>計算式が設定されたセルのコピー・移動を行った場合、参照セルの座標は自動調整が行われます。</p>
					<p>
						<input type="submit" name="templateopen" value="テンプレートを確認" style="width: 150px;" />
					</p>
					<p>
						<input type="submit" name="calculate2" value="Excel ファイル作成" style="width: 150px;" />
					</p>
					<p class="annotation1">※サンプルコードは「src/servlet/CalculateServlet.java」を参照してください。</p>
